...
//  kuoyi
//
//  Created by alexzinder on 2018/7/12.
...
@property (nonatomic, strong) UITextField *regConfirmPwdTF;
...
-(void)p_initUI{
    ...
    [self.view addSubview:self.regPwdTF];
    ...
    self.regConfirmPwdTF = [[UITextField alloc] init];
    [self.view addSubview:self.regConfirmPwdTF];
    self.regConfirmPwdTF.placeholder = @"确认密码";
    self.regConfirmPwdTF.secureTextEntry = YES;
    self.regConfirmPwdTF.textColor = [UIColor colorWithHexString:@"a9acab"];
    self.regConfirmPwdTF.font = [UIFont systemFontOfSize:15];
    [self.regConfirmPwdTF mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.regPwdTF.mas_bottom).offset(tfSeg);
        make.left.equalTo(self.phoneTF.mas_left);
        make.right.equalTo(self.phoneTF.mas_right);
        make.size.height.mas_equalTo(tfHeight);
    }];
    ...
    [self.changeBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.equalTo(self.phoneTF.mas_left);
        make.right.equalTo(self.phoneTF.mas_right);
        make.top.equalTo(self.regConfirmPwdTF.mas_bottom).offset(19 + 18);
    }];
...
-(void)p_confirmAction{
    ...
    }else if ([self.regConfirmPwdTF.text isEqualToString:@""]){
        [HLYHUD showHUDWithMessage:@"请先填写确认密码" addToView:nil];
    }else if (![self.regPwdTF.text isEqualToString:self.regConfirmPwdTF.text]){
        [HLYHUD showHUDWithMessage:@"请填写相同密码" addToView:nil];
    }else if ([self.codeTF.text isEqualToString:@""]){
        [HLYHUD showHUDWithMessage:@"请先填写验证码" addToView:nil];
    }else{
        [weakSelf changePwdRequest];
    }
...
-(void)sendCodeRequest{
    ...
    NSDictionary *params = @{@"phone":self.phoneTF.text,@"type":@(FindPWDCode)};
...
-(void)changePwdRequest{
    ...
    __weak typeof(self)weakSelf = self;
    NSDictionary *params = @{@"phone":self.phoneTF.text,@"password":self.regPwdTF.text,@"code":self.codeTF.text};
