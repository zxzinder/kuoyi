...
- (void)initView {
    ...
    self.pickerBackView = [[UIView alloc] initWithFrame:CGRectMake(0, self.frame.size.height-(250+30), self.width, 250+30)];
    self.pickerBackView.backgroundColor = [UIColor whiteColor];
    UIButton *cancelBtn = [[UIButton alloc] initWithFrame:CGRectMake(5, 10, 50, 30)];
    ...
    UIButton *ensureBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.pickerBackView.width-55, 10, 50, 30)];
    [ensureBtn setTitleColor:[UIColor colorWithHexString:@"17a7af"] forState:UIControlStateNormal];
    ...
    UIView *segview = [[UIView alloc] initWithFrame:CGRectMake(0, 50, self.width, 1)];
    segview.backgroundColor = [UIColor colorWithHexString:@"e6e6e6"];
    [self.pickerBackView addSubview:segview];
    self.myPicker = [[UIPickerView alloc] initWithFrame:CGRectMake(0, 30, self.width, 250)];
...
- (void)getPickerData {
    NSString *path = [[NSBundle mainBundle] pathForResource:@"citys.json" ofType:nil];

    NSArray *provenceArr = [NSJSONSerialization JSONObjectWithData:[NSData dataWithContentsOfFile:path] options:NSJSONReadingMutableLeaves error:nil];
    self.provinceArray = provenceArr;
    self.selectedArray = self.provinceArray[0][@"child"];
    if (self.selectedArray.count > 0) {
        self.cityArray = [self.provinceArray objectAtIndex:0][@"child"];
    }
    if (self.cityArray.count > 0) {
        self.districtArray = [self.cityArray objectAtIndex:0][@"child"];
    }
...
- (NSString *)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component {
    if (component == 0) {
        ...
        return [self.districtArray objectAtIndex:row][@"name"];
    }
}
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
        if (component == 0) {
            self.selectedArray = [self.provinceArray objectAtIndex:row][@"child"];
            if (self.selectedArray.count > 0) {
                self.cityArray = [self.provinceArray objectAtIndex:row][@"child"];
            } else {
                self.cityArray = nil;
            }
            if (self.cityArray.count > 0) {
        ...
        if (component == 1) {
            if (self.selectedArray.count > 0 && self.cityArray.count > 0) {
                self.districtArray = [self.cityArray objectAtIndex:row][@"child"];
            } else {
                self.districtArray = nil;
            }
            [pickerView selectRow:0 inComponent:2 animated:NO];
...
//    }
//    [self hidePicker];
    NSString *province = [self.provinceArray objectAtIndex:[self.myPicker selectedRowInComponent:0]][@"name"];
    NSString *city = [self.cityArray objectAtIndex:[self.myPicker selectedRowInComponent:1]][@"name"];
    NSString *district = [self.districtArray objectAtIndex:[self.myPicker selectedRowInComponent:2]][@"name"];
    NSString *code = [self.districtArray objectAtIndex:[self.myPicker selectedRowInComponent:2]][@"code"];
    NSLog(@"%@%@%@",province,city,district);
    if (self.refreshInfoBlock) {
        self.refreshInfoBlock([NSString stringWithFormat:@"%@,%@,%@",province,city,district],code);
    }
    [self hidePicker];
}
